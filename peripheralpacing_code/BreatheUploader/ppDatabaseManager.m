...
+(ppUser *)jsonToUser:(NSString *)info
{
	...
	SBJsonParser *parser = [[SBJsonParser alloc] init];
	NSDictionary *statuses = [parser objectWithString:info error:nil];
	...
	[newUser setBpm:[[statuses objectForKey:@"bpm"] floatValue]];
	[newUser setRestingRate:[[statuses objectForKey:@"restingrate"] floatValue]];

	[newUser setName:[statuses objectForKey:@"name"]];
	[newUser setImageURL:[statuses objectForKey:@"url"]];
	[newUser setPoints:[[statuses objectForKey:@"points"] integerValue]];
	NSString *dateStr = [statuses objectForKey:@"lastUpdated"];
...
// gets list of all names from database. 
+(NSArray *)getAllUsers {
	NSString *rootpath = ROOT;
	NSString *subpath = USERINFOPATH;
	...
	// return should contain only NSStrings of user names
	SBJsonParser *parser = [[SBJsonParser alloc] init];
	NSArray *allUsersArray = [parser objectWithString:returnStr];
	[parser release];
