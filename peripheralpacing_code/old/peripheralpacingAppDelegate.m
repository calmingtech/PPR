...
- (void)setupAvatars
{
	...
	while(allocated<NUM_USERS)
	{
		...
		//[usersIcons[allocated] retain];
		ids[allocated] = id;
		//printf("%s\n",[[NSString stringWithFormat:@"%s",names[id]] UTF8String]);
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
	...
	blankImage = [[NSImage alloc] initWithContentsOfFile:[graphics_dir stringByAppendingString:@"/blank.png"]];
...
-(void)awakeFromNib{
	...
	[statusItem setTitle:@"CrowdRespiration"];
	[statusItem setHighlightMode:YES];
	
	start_stop = [statusMenu addItemWithTitle:@"Start" action:@selector(toggle_on_off:) keyEquivalent:@""];
	SendModeItem = [statusMenu addItemWithTitle:@"Enable Sending" action:@selector(toggle_send:) keyEquivalent:@""];
...
- (void) mainloop:(NSConnection *)connection
{
	...
	while(true)
	{
		...
		//printf("opened\n");
		fprintf(fp,"%f %d %f %d %f %d %f\n",timePassed_s,val,breathrate,running,baseline_bpm,cycle,notification_time);
		//printf("appended\n");
		fclose(fp);
		...
		if (running)
		{
			...
			if (!cycle)
			{
				if ((breathrate>baseline_bpm*((100+percentAboveBaseline)/100.0)) && (cycles_since_interval<1))
				{
					cycle = true;
			...
			if ((int)floor(timePassed_s)%240==0) //this assumes multiple samples come in per second, which might not be true, but if it isn't then nothing will work anyway
			{
